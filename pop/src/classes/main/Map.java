...
public class Map implements IPositionChangedObserver{
    ...
    private final Vector2d jungleLoweLeft;
    // --
    private final int mapSurface;
    private final int jungleSurface;
    private final int sawannaSurface;
    // --
    ...
    public boolean birthAnimal(Animal father, Animal mother, Vector2d position){
        ...
        if(this.objectsAround(position) < 8){
            ...
            // mamy dobrą pozycje
            Animal kid = new Animal(this,kidPosition,father.getEnergy()/4 + mother.getEnergy()/4,this.initialEnergy,kidDna,this.moveEnergy);
            father.setEnergy(father.getEnergy() - father.getEnergy()/4);
            father.addKid(kid);
            mother.setEnergy(mother.getEnergy() - mother.getEnergy()/4);
            mother.addKid(kid);
            this.animalHashMapList.put(kidPosition,kid);
            kid.addObserver(this);
            father.increaseKidCount();
            mother.increaseKidCount();
            return true;
        }
    ...
    public void eatAndRemoveGrass(Animal animal, Vector2d position){ // animal -> grass eater
        ...
        animal.increaseEnergy(grass.getEnergyValue());
    ...
    public LinkedList<Animal> getStrongestTwo(LinkedList<Animal> animalList){
        ...
        for(Animal animal: animalList){
            if(animal.getEnergy() >= maxEnergy){
                father = animal;
                maxEnergy = father.getEnergy();
            }
        }
        maxEnergy = 0;
        for(Animal animal: animalList){
            if(animal.getEnergy() >= maxEnergy && !animal.equals(father)){
                mother = animal;
                maxEnergy = mother.getEnergy();
            }
        }
    ...
    public void breeding(){
        ...
        for(Animal animal: AnimalList){
            ...
            if(animalsAt >= 2){
                ...
                if(father.getEnergy() >= this.initialEnergy/2 && mother.getEnergy() >= this.initialEnergy/2){
